...
import { FunctionLoggerOptions } from './options';
...
const getArgsStrings = function (argValues: any[], func: Function, options: FunctionLoggerOptions): string[] {
    ...
    }).map((argNameIndex: number) => {
        if (argNameIndex === -1 || argNameIndex >= argValues.length || !argNames) return '';
...
export const logMessage = function (isStart: boolean, targetInstance, functionName, originalFunction, functionArgsVals, options: FunctionLoggerOptions): void {
    ...
    const args: any = options.withArgs ? getArgsStrings(functionArgsVals, originalFunction, options) : null;
    const props: any = options.withClassProperties ? getPropertiesStrings(options.withClassProperties, targetInstance) : null;
